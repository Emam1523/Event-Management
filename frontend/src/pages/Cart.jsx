...
const Cart = () => {
  ...
  if (cartItems.length === 0) {
    return (
      <div className="min-h-screen bg-dark-bg flex items-center justify-center text-center px-4">
        <div className="glass-card p-16 max-w-lg bg-white/5 border-white/5 rounded-[3rem]">
          ...
          <h2 className="text-3xl font-black text-white mb-4 uppercase tracking-tighter">Your Bag is Empty</h2>
          <p className="text-slate-400 mb-10 font-medium leading-relaxed">
            Looks like you haven't secured any passes yet. Explore the hottest events happening soon!
          </p>
  ...
  return (
    <div className="min-h-screen bg-dark-bg pt-32 pb-24 text-slate-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="flex flex-col md:flex-row gap-8">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex justify-between items-start">
                      ...
                      <button
                        ...
                        className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-slate-500 hover:text-rose-500 hover:bg-rose-500/10 transition-all border border-white/5"
                      >
                        <FiTrash2 />
                      </button>
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-slate-500 text-xs font-bold uppercase tracking-widest">
                      <span>{format(new Date(item.event.date), 'MMM d, yyyy')}</span>
                  ...
                  <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-4 md:min-w-[120px] pt-4 md:pt-0 border-t md:border-t-0 border-white/5">
                    <div className="flex items-center bg-white/5 rounded-2xl border border-white/5 p-1">
                      <button
                        onClick={() => updateQuantity(item.event.id, item.ticketType.id, item.quantity - 1)}
          ...
          <div className="lg:col-span-4">
            <div className="glass-card p-10 bg-slate-900 border-white/10 sticky top-32 shadow-3xl shadow-black/40">
              <h2 className="text-xs font-black uppercase tracking-[0.4em] text-slate-500 mb-10 text-center">ORDER SUMMARY</h2>

              <div className="space-y-6 mb-10">
                <div className="flex justify-between items-center">
                  <span className="text-xs font-black text-slate-500 uppercase tracking-widest">Subtotal</span>
                  <span className="text-lg font-bold text-white">৳{getCartTotal().toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center">
              ...
              <button
                ...
                className="w-full btn-primary py-5 rounded-[2rem] text-[11px] mb-6 shadow-2xl shadow-primary/20"
              >
                Proceed to Checkout
              </button>

              <Link to="/events" className="block text-center text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-white transition-colors">
                Continue Shopping
              </Link>
