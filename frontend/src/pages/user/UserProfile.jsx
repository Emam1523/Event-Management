...
const UserProfile = () => {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    const payload = {
      ...
      ...(formData.passwordVerificationCode
        ? { passwordVerificationCode: formData.passwordVerificationCode }
        : {}),
    };
    const result = await updateProfile(payload);
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      showNotification(
        myReview ? "Review updated!" : "Review submitted! Thank you ✨",
        "success",
      );
    } catch {
  ...
  return (
    <div className="min-h-screen bg-[#05050a] text-zinc-200 pt-28 pb-24 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          <div className="h-40 bg-linear-to-r from-[#1a0a00] via-[#1a0020] to-[#00101a] relative">
            <div className="absolute inset-0 bg-linear-to-r from-brand-orange/20 via-violet-500/20 to-cyan-500/20" />
            {[...Array(6)].map((_, i) => (
              <motion.div
                key={i}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      {isAdmin && (
                        <span className="px-3 py-1 rounded-full bg-brand-orange/15 border border-brand-orange/30 text-brand-orange text-[9px] font-black uppercase tracking-widest">
                          Admin
                        </span>
                      )}
                    </div>
                    ...
                    {user?.phone && (
                      <p className="text-zinc-600 text-xs mt-1">{user.phone}</p>
                    )}
                  </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-8">
              {statCards.map((s, i) => (
                <motion.div
                  ...
                  className="relative rounded-2xl bg-white/3 border border-white/8 p-4 overflow-hidden group hover:border-white/15 transition-all"
                >
                  <div
                    className={`absolute inset-0 bg-linear-to-br ${s.color} opacity-0 group-hover:opacity-5 transition-opacity`}
                  />
                  <div
                    className={`w-8 h-8 rounded-xl bg-linear-to-br ${s.color} flex items-center justify-center text-white text-sm mb-3`}
                  >
                    {s.icon}
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          >
            {/* Tab Navigation */}
            <div className="rounded-4xl bg-zinc-950/90 border border-white/8 p-3 space-y-1">
              <p className="text-[9px] font-black uppercase tracking-[0.25em] text-zinc-600 px-3 pb-2">
                Navigation
              </p>
              {tabs.map((t) => (
                <button
            ...
            {/* Account badge */}
            <div className="rounded-4xl bg-linear-to-br from-brand-orange/10 to-violet-600/10 border border-brand-orange/20 p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <p className="text-xs font-black text-white">
                  {isAdmin ? "Administrator" : "NEXTDHAKA Member"}
                </p>
              </div>
              <p className="text-[10px] text-zinc-500 font-medium leading-relaxed">
                {isAdmin
                  ? "You have full platform access and management privileges."
                  : "Enjoy exclusive access to premium events across Bangladesh."}
              </p>
            </div>
          </motion.aside>
            ...
            <AnimatePresence mode="wait">
              ...
              {activeTab === "profile" && (
                <motion.form
                  ...
                  exit={{ opacity: 0, x: -20 }}
                  className="rounded-4xl bg-zinc-950/90 border border-white/8 p-8 md:p-10 shadow-2xl"
                >
                  <div className="flex items-center gap-4 mb-8 pb-6 border-b border-white/5">
                    ...
                    {isEditing && (
                      <span className="ml-auto px-3 py-1 rounded-full bg-brand-orange/15 border border-brand-orange/30 text-brand-orange text-[9px] font-black uppercase tracking-widest animate-pulse">
                        Editing
                      </span>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div
                        className={`flex items-center rounded-2xl border bg-white/3 px-4 py-1 transition-all ${isEditing ? "border-white/15 focus-within:border-brand-orange/50 focus-within:ring-2 focus-within:ring-brand-orange/10" : "border-white/5"}`}
                      >
                        <FiUser
                          className={`shrink-0 ${isEditing ? "text-brand-orange" : "text-zinc-600"}`}
                        />
                        <input
                          type="text"
                          name="name"
                          ...
                          disabled={!isEditing}
                          required
                          className="w-full bg-transparent border-none outline-none py-3 px-3 text-white disabled:text-zinc-500 font-semibold text-sm"
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center rounded-2xl border border-white/5 bg-white/2 px-4 py-1 opacity-60 cursor-not-allowed">
                        <FiMail className="shrink-0 text-zinc-600" />
                        <input
                          type="email"
                          value={user?.email || ""}
                          readOnly
                          className="w-full bg-transparent border-none outline-none py-3 px-3 text-zinc-500 font-semibold text-sm cursor-not-allowed"
                        />
                        <FiLock size={12} className="text-zinc-700 shrink-0" />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 md:col-span-2">
                      ...
                      >
                        ...
                        <input
                          ...
                          className="w-full bg-transparent border-none outline-none py-3 px-3 text-white disabled:text-zinc-500 placeholder:text-zinc-700 font-semibold text-sm"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Account status */}
                  <div className="mt-8 p-5 rounded-2xl bg-white/2 border border-white/5 flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-white mt-0.5">
                        ...
                        <span className="text-zinc-500 font-medium text-xs ml-3">
                ...
                >
                  <div className="flex items-center gap-4 mb-8 pb-6 border-b border-white/5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 mt-0.5">
                        Change your password — must be strong
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        <FiLock
                          className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-violet-400 transition-colors"
                          size={18}
                        />
                        <input
                          ...
                          disabled={!isEditing}
                          placeholder="••••••••"
                          className="w-full pl-14 pr-14 py-4 bg-white/5 border border-white/5 rounded-2xl text-white font-bold placeholder:text-zinc-700 focus:outline-none focus:border-violet-500/50 transition-all focus:bg-white/8 disabled:opacity-50 disabled:cursor-not-allowed"
                        />
                        <button
                          type="button"
                          disabled={!isEditing}
                          className="absolute right-5 top-1/2 -translate-y-1/2 text-zinc-600 hover:text-white transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        <FiCheckCircle
                          className={`absolute left-5 top-1/2 -translate-y-1/2 transition-colors ${formData.confirmPassword && formData.password === formData.confirmPassword ? "text-emerald-400" : "text-zinc-600 group-focus-within:text-violet-400"}`}
                          size={18}
                        />
                        <input
                        ...
                        >
                          ...
                          <div className="flex items-center rounded-2xl border border-brand-orange/40 bg-brand-orange/5 px-4 py-1">
                            ...
                            <input
                              ...
                              className="w-full bg-transparent border-none outline-none py-3 px-3 text-white placeholder:text-zinc-600 font-semibold text-sm tracking-widest disabled:opacity-50 disabled:cursor-not-allowed"
                            />
                          </div>
              ...
              {activeTab === "activity" && !isAdmin && (
                <motion.div
                  ...
                  className="space-y-6"
                >
                  <div className="rounded-4xl bg-zinc-950/90 border border-white/8 p-8 md:p-10 shadow-2xl">
                    ...
                    {loadingBookings ? (
                      ...
                      <div className="text-center py-16">
                        <div className="w-20 h-20 rounded-3xl bg-white/5 flex items-center justify-center mx-auto mb-5">
                          <BsTicketPerforated className="text-4xl text-zinc-600" />
                        </div>
                        ...
                        <Link
                          to="/events"
                          className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-brand-orange text-white text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all"
                        >
                        ...
                        {recentBookings.map((booking, i) => {
                          ...
                          return (
                            <motion.div
                              key={booking.id}
                              onClick={() =>
                                navigate(`/my-tickets/${booking.id}`)
                              }
                              initial={{ opacity: 0, y: 12 }}
                              animate={{ opacity: 1, y: 0 }}
                              transition={{ delay: i * 0.1 }}
                              className="flex items-center gap-5 p-4 rounded-2xl bg-white/3 border border-white/5 hover:border-white/12 hover:bg-white/5 transition-all group cursor-pointer"
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="font-black text-white text-sm truncate group-hover:text-brand-orange transition-colors">
                              ...
                              <div className="text-right shrink-0">
                                ...
                                <span
                                  className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full mt-1.5 inline-block ${isUpcoming ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/20" : "bg-white/5 text-zinc-500 border border-white/5"}`}
                                >
                  ...
                  <div className="rounded-4xl bg-linear-to-br from-brand-orange/10 via-rose-500/5 to-violet-600/10 border border-brand-orange/15 p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
                    <div>
                      <h3 className="text-xl font-black text-white">
                        Ready for the next experience?
                      </h3>
                      <p className="text-zinc-400 text-sm mt-1">
                        Discover premium events happening across Bangladesh.
                      </p>
                    </div>
                    <Link
                      to="/events"
                      className="shrink-0 inline-flex items-center gap-3 px-8 py-4 rounded-2xl bg-brand-orange text-white text-[11px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all shadow-lg shadow-brand-orange/25"
                    >
                ...
                >
                  <div className="flex items-center gap-4 mb-8 pb-6 border-b border-white/5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 mt-0.5">
                        Share your experience with the platform — shown on the
                        home page
                      </p>
                    </div>
                  </div>
                  ...
                  {myReview && !reviewEditing ? (
                    ...
                    <div className="space-y-6">
                      <div className="p-6 rounded-2xl bg-white/3 border border-amber-500/20">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 text-zinc-400 hover:text-white text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all cursor-pointer"
                          >
                        ...
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <span className="text-amber-400 font-black text-lg ml-1">
                            {myReview.rating}/5
                          </span>
                        </div>
                        ...
                        <p className="text-zinc-600 text-[10px] font-medium mt-4">
                          Submitted{" "}
                          {isValid(new Date(myReview.createdAt))
                            ? format(
                                new Date(myReview.createdAt),
                                "MMM d, yyyy",
                              )
                            : ""}
                        </p>
                      ...
                      <p className="text-zinc-500 text-sm text-center">
                        Thank you for sharing your feedback! ✨ It's displayed
                        on the home page.
                      </p>
                    </div>
                  ) : (
                    ...
                    <form onSubmit={handleReviewSubmit} className="space-y-8">
                      ...
                      <div className="space-y-2">
                        ...
                        <textarea
                          ...
                          className="w-full rounded-2xl border border-white/10 bg-white/3 px-5 py-4 text-white placeholder:text-zinc-700 font-semibold text-sm focus:outline-none focus:border-amber-500/40 focus:ring-2 focus:ring-amber-500/10 resize-none transition-all"
                        />
