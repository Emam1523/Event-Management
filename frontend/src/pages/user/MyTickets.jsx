...
import {
  FiX, FiMapPin, FiClock, FiSearch,
  FiArrowRight, FiTag, FiUsers, FiArrowLeft,
} from 'react-icons/fi';
...
const DEFAULT_IMG = 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=800';

const statusConfig = {
  confirmed: { label: 'Confirmed', className: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/25', dot: 'bg-emerald-400' },
  ...
  used:      { label: 'Used',      className: 'bg-white/5 text-slate-500 border-white/8',             dot: 'bg-slate-500'   },
};
...
const MyTickets = () => {
  ...
  const displayed = (activeTab === 'upcoming' ? upcoming : past)
    .filter((t) => !search || t.event.title.toLowerCase().includes(search.toLowerCase()));
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#05050a]">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 text-sm font-medium animate-pulse">Loading your passes…</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#05050a] pt-28 pb-24 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10 space-y-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <Link to="/profile" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-white transition-colors group">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/8 text-[9px] font-black uppercase tracking-widest text-slate-400">
              <HiSparkles className="text-brand-orange" /> Pass Archive
            </div>
          </div>
          <h1 className="text-4xl sm:text-5xl font-black text-white tracking-tighter">My Passes</h1>
          <p className="text-slate-500 font-medium">Your collection of world-class experiences</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
          ...
          {[
            { label: 'Total Passes', value: tickets.length, color: 'from-brand-orange to-rose-500' },
            { label: 'Upcoming', value: upcoming.length, color: 'from-violet-500 to-purple-600' },
            { label: 'Past Events', value: past.length, color: 'from-emerald-500 to-teal-500' },
          ].map((s) => (
            <div key={s.label} className="rounded-2xl bg-white/[0.03] border border-white/8 p-5">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}
          ...
          <div className="flex gap-1 p-1.5 rounded-2xl bg-zinc-950 border border-white/8">
            {[
              ...
              <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                ...
                {activeTab === tab.key && (
                  <motion.div layoutId="ticket-tab"
                    className="absolute inset-0 bg-white/5 border border-white/10 rounded-xl"
                    transition={{ type: 'spring', bounce: 0.2, duration: 0.5 }} />
                )}
                <span className="relative z-10">{tab.label}</span>
                <span className={`relative z-10 px-2 py-0.5 rounded-lg text-[9px] font-black ${activeTab === tab.key ? 'bg-brand-orange text-white' : 'bg-white/5 text-slate-600'}`}>
                  {tab.count}
                </span>
              </button>
            ))}
          ...
          <div className="relative flex-1 max-w-sm ml-auto">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
              ...
              className="w-full bg-white/[0.03] border border-white/8 rounded-2xl pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-slate-700 focus:outline-none focus:border-brand-orange/40 font-medium transition-all" />
          </div>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeTab}
            ...
            {displayed.length === 0 ? (
              <div className="py-24 text-center rounded-[2.5rem] border border-dashed border-white/8">
                <div className="w-20 h-20 rounded-3xl bg-white/5 flex items-center justify-center mx-auto mb-6 text-3xl text-slate-700">
                  <BsQrCode />
                </div>
                <h2 className="text-2xl font-black text-white mb-3 tracking-tight">No Passes Found</h2>
                <p className="text-slate-500 text-sm mb-8 max-w-xs mx-auto font-medium">
                  {search ? 'No tickets match your search.' : activeTab === 'upcoming' ? "You haven't secured any upcoming event passes yet." : "You haven't attended any events yet."}
                </p>
                <Link to="/events"
              ...
              displayed.map((ticket, i) => {
                ...
                const sc = statusConfig[ticket.status] || statusConfig.confirmed;
                const isUpcoming = d >= now;
                return (
                  <motion.div key={ticket.id}
                    initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                    className="group relative rounded-[2rem] overflow-hidden border border-white/8 hover:border-brand-orange/25 transition-all duration-500 bg-zinc-950/60 backdrop-blur-xl">

                    {/* Hover glow */}
                    <div className="absolute inset-0 bg-gradient-to-r from-brand-orange/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />

                    <div className="flex flex-col md:flex-row">
                      {/* Image side */}
                      <div className="md:w-56 lg:w-72 h-48 md:h-auto relative overflow-hidden shrink-0">
                        <img src={imgSrc} alt={ticket.event.title}
                      ...
                      <div className="flex-1 p-6 md:p-8 flex flex-col justify-between">
                        <div>
                          ...
                          <div className="flex flex-wrap items-center gap-3 mb-4">
                            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${sc.className}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${sc.dot}`} /> {sc.label}
                            </span>
                            {ticket.event.category && (
                              <span className="px-3 py-1.5 rounded-full bg-white/5 border border-white/8 text-[9px] font-black uppercase tracking-widest text-slate-400">
                                {ticket.event.category}
                              </span>
                            )}
                            <span className="ml-auto text-[9px] font-mono text-slate-600 font-bold tracking-widest">
                              {ticket.bookingId}
                            </span>
                          </div>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            {[
                              { icon: <FiClock size={12} />, label: 'Time', val: ticket.event.time },
                              { icon: <FiMapPin size={12} />, label: 'Venue', val: ticket.event.location.split(',')[0] },
                              { icon: <FiTag size={12} />, label: 'Type', val: ticket.ticketType },
                              { icon: <FiUsers size={12} />, label: 'Passes', val: `${ticket.quantity}x` },
                            ].map((item) => (
                              <div key={item.label}>
                                <p className="text-[8px] font-black uppercase tracking-widest text-slate-600 mb-1">{item.label}</p>
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-4 mt-6 pt-5 border-t border-white/5">
                          <div>
                            <p className="text-[8px] font-black text-slate-600 uppercase tracking-widest mb-1">Total Paid</p>
                            <p className="text-2xl font-black text-white tracking-tight">৳{ticket.totalPrice.toLocaleString()}</p>
                          </div>
                    ...
                    <div className="absolute right-0 top-1/2 -translate-y-1/2 hidden md:flex flex-col gap-1.5 -mr-[3px]">
                      {[...Array(5)].map((_, i) => <div key={i} className="w-1.5 h-3 bg-[#05050a] rounded-full" />)}
                    </div>
                  </motion.div>
                );
              })
          ...
          {selectedTicket && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                className="relative w-full max-w-sm overflow-hidden rounded-[2.5rem] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.8)]"
              >
                {/* Ticket top section */}
                <div className="bg-gradient-to-br from-brand-orange to-rose-600 p-7 relative">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest text-white/60">Entry Pass</span>
                  </div>
                  <h2 className="text-xl font-black text-white leading-tight mb-4 pr-8">{selectedTicket.event.title}</h2>
                  <div className="grid grid-cols-2 gap-3 pt-4 border-t border-white/20">
                    <div>
                      <p className="text-[8px] font-black uppercase text-white/50 mb-0.5">Venue</p>
                      <p className="text-xs font-bold text-white truncate">{selectedTicket.event.location}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[8px] font-black uppercase text-white/50 mb-0.5">Time</p>
                      <p className="text-xs font-bold text-white">{selectedTicket.event.time}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-zinc-950 p-7 flex flex-col items-center">
                  <div className="p-4 bg-white rounded-2xl shadow-2xl mb-5">
                    <QRCodeSVG value={selectedTicket.qrData} size={180} level="H" includeMargin={false} />
                  </div>
                  <p className="text-base font-mono font-bold text-white tracking-[0.3em] mb-1">{selectedTicket.bookingId}</p>
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-6">Scan at entry gate</p>

                  <div className="grid grid-cols-3 gap-4 w-full text-center border-t border-b border-zinc-800/60 py-4 mb-6">
                    {[
                      { label: 'Type', val: selectedTicket.ticketType.split(' ')[0] },
                      ...
                      <div key={label}>
                        ...
                        <p className="text-xs font-black text-white capitalize">{val}</p>
                      </div>
                    ))}
                  </div>

                  <p className="text-2xl font-black text-white">৳{selectedTicket.totalPrice.toLocaleString()}</p>
                  <p className="text-[9px] text-slate-600 font-medium mt-1">Total Paid</p>
                </div>
              </motion.div>
            </div>
          )}
