...
import { FiMail, FiShield, FiArrowRight, FiRefreshCw, FiAlertCircle } from 'react-icons/fi';
...
const VerifyEmail = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const msg = error.response?.data?.message || 'Verification failed. Incorrect code.';
      setErrorMsg(msg);
  ...
  return (
    <div className="min-h-screen w-full bg-[#030303] flex items-center justify-center p-4 pt-24 relative overflow-hidden">
      ...
      <motion.div
        ...
        className="w-full max-w-lg rounded-[2.5rem] border border-white/10 bg-white/[0.03] backdrop-blur-3xl p-6 md:p-8 shadow-2xl shadow-black/50 text-center relative mx-auto my-auto"
      >
        ...
        <h1 className="text-xl md:text-2xl font-black text-white tracking-tight mb-2 uppercase">{title}</h1>
        ...
        <p className="text-zinc-400 font-medium mb-5 text-sm px-4">{description}</p>
        ...
        >
          <div className="space-y-3 text-left">
            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em] ml-1">Secure Code</label>
            <input
              type="text"
              inputMode="numeric"
              value={code}
