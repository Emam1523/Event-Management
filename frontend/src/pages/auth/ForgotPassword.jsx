...
import {
  FiMail, FiShield, FiArrowRight, FiLock, FiCheck,
  FiRefreshCw, FiKey, FiAlertCircle, FiArrowLeft
} from 'react-icons/fi';
        ...
        <div className="flex items-center justify-center gap-3 mb-8">
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center gap-3">
              ...
              {s < 3 && (
                <div className={`w-12 h-[2px] rounded-full ${step > s ? 'bg-brand-orange' : 'bg-white/10'}`} />
              )}
            </div>
          ))}
        </div>

        <motion.div className="rounded-[2.5rem] border border-white/10 bg-white/[0.03] backdrop-blur-3xl p-8 md:p-10 shadow-2xl shadow-black/50 relative overflow-hidden">
                ...
                >
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      <FiMail className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-brand-orange transition-colors" />
                      <input
                        type="email"
                        required
                        value={email}
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.99 }}
                    className="w-full bg-gradient-to-r from-brand-orange to-brand-secondary text-white font-black py-5 rounded-2xl uppercase tracking-[0.25em] text-xs transition-all flex items-center justify-center gap-3 group disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-brand-orange/20"
                  >
                    {isLoading ? (
                      ...
                      <>
                        Send Code <FiArrowRight className="group-hover:translate-x-1 transition-transform" />
                      </>
                    )}
                  </motion.button>
                </motion.form>
                ...
                >
                  <div className="flex flex-col items-center mb-4">
                    <div className="relative w-20 h-20 mb-3">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className={`text-2xl font-black ${timer > 10 ? 'text-white' : 'text-red-400'}`}>
                          {timer}
                        </span>
                      </div>
                    </div>
                  ...
                  <AnimatePresence>
                    {codeError && (
                      ...
                      >
                        ...
                        <p className="text-[11px] font-bold text-red-300 tracking-wider uppercase">{codeError}</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                  ...
                  <div className="flex flex-col gap-3 pt-2">
                    <motion.button
                      ...
                      whileHover={{ scale: 1.01 }}
                      whileTap={{ scale: 0.99 }}
                      className="w-full bg-gradient-to-r from-brand-orange to-brand-secondary text-white font-black py-5 rounded-2xl uppercase tracking-[0.25em] text-xs transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-brand-orange/20"
                    >
                      {isLoading ? (
                        <span className="flex items-center gap-2">
                          <svg className="animate-spin h-5 w-5" viewBox="0 0 24 24" fill="none">
              ...
              {step === 3 && (
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      <FiLock className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-brand-orange transition-colors" />
                      <input
                        type="password"
                        required
                        value={password}
            ...
            <div className="mt-8 flex items-center justify-between">
              {step > 1 ? (
                <button
                  type="button"
                  onClick={goBack}
                  className="flex items-center gap-2 text-zinc-600 hover:text-white transition-colors text-xs font-black uppercase tracking-widest group"
                >
