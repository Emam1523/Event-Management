    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4">
        <div>
          ...
          <p className="text-gray-400 font-medium italic">Deciphering the elite economy of your gatherings.</p>
        </div>
      </div>
        ...
        {statCards.map((card) => {
          return (
            <motion.div 
              ...
              className="bg-white/5 border border-white/10 rounded-[2rem] p-8 backdrop-blur-xl relative group hover:bg-white/[0.08] transition-all overflow-hidden"
            >
              ...
              <div className="flex items-center justify-between mb-6 relative z-10">
                <div className={`${card.bg} ${card.color} w-12 h-12 rounded-2xl flex items-center justify-center text-xl`}>
                  {card.icon}
                </div>
              </div>
              <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-1 relative z-10">{card.label}</p>
              <p className="text-3xl font-black text-white group-hover:scale-105 origin-left transition-transform relative z-10">{card.value}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <motion.div 
          variants={itemVariants}
          className="lg:col-span-2 bg-white/5 border border-white/10 rounded-[2.5rem] p-10 backdrop-blur-xl relative overflow-hidden"
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10">
            <div>
              ...
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">Financial & Volume Metrics Over Time</p>
            </div>
        ...
        >
          ...
          <p className="text-[10px] text-gray-500 font-black uppercase tracking-[0.2em] mb-8">Top performing exhibitions</p>
          ...
          <div className="space-y-6">
            {topEvents.map((event, index) => (
              ...
              >
                <div className={`w-12 h-12 rounded-2xl ${event.color} flex items-center justify-center font-black text-white shadow-xl flex-shrink-0`}>
                  {index + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-black text-white text-sm truncate">{event.name}</p>
                  <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest">{event.category}</p>
                </div>
                <div className="text-right">
                  <p className="font-black text-brand-orange text-sm">৳{(event.revenue/1000).toFixed(1)}K</p>
                  <p className="text-[9px] text-gray-600 font-bold uppercase">{event.bookings} Sold</p>
                </div>
              </motion.div>
            ))}
          </div>
          
          <button className="w-full mt-10 py-5 rounded-[2rem] border border-white/10 text-[10px] font-black text-white hover:bg-white/5 uppercase tracking-widest transition-all">
            View Full Report
          </button>
        </motion.div>
      </div>
