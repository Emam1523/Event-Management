...
const AdminReviews = () => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-black text-white tracking-tight">App Reviews</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="rounded-2xl bg-zinc-900/80 border border-white/8 p-6">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-3">Total Reviews</p>
          <p className="text-5xl font-black text-white">{reviews.length}</p>
        </div>
        <div className="rounded-2xl bg-zinc-900/80 border border-white/8 p-6">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4">Rating Breakdown</p>
          <div className="space-y-2">
            {distribution.map(({ star, count, pct }) => (
              <div key={star} className="flex items-center gap-3">
                <span className="text-[10px] font-black text-slate-500 w-3">{star}</span>
                <FiStar size={10} className="text-amber-400" style={{ fill: 'currentColor' }} />
      ...
      <div className="rounded-2xl bg-zinc-900/80 border border-white/8 overflow-hidden">
        <div className="p-6 border-b border-white/5 flex items-center gap-3">
          ...
          <span className="ml-auto text-[10px] bg-white/5 text-slate-400 px-3 py-1 rounded-full font-bold">{reviews.length} total</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-16 text-center">
            <FiMessageSquare className="text-slate-700 text-4xl mx-auto mb-4" />
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-3 mb-1.5">
                    <p className="font-black text-white text-sm">{review.user?.name || 'Anonymous'}</p>
                    <StarDisplay rating={review.rating} />
                    <span className="text-[10px] text-slate-600 font-medium ml-auto">
                      {isValid(new Date(review.createdAt)) ? format(new Date(review.createdAt), 'MMM d, yyyy') : ''}
                    </span>
                  </div>
