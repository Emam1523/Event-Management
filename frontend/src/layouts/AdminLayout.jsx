      ...
      >
        <div className="p-8 flex items-center gap-4">
           <button 
             ...
             className="w-10 h-10 bg-brand-orange rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg shadow-brand-orange/20 hover:scale-105 active:scale-95 transition-transform"
           >
              <FiShield className="text-xl text-white" />
           </button>
           {!isCollapsed && (
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col">
                <span className="text-lg font-black tracking-tighter">Admin Portal</span>
             </motion.div>
           )}
        </div>
               ...
               <nav className="flex-1 space-y-2">
                 {navItems.map((item) => {
                   ...
                   return (
                     ...
                     >
                       <span className={`text-xl ${active ? 'text-brand-orange' : ''}`}>{item.icon}</span>
