...
exports.getAnalytics = asyncHandler(async (req, res) => {
  try {
    ...
    if (timeframe === 'month') {
      ...
      revenueData = Object.keys(daily).map(key => ({ 
    ...
    const processedTopEvents = topEvents.map((e, index) => {
      const eventRevenue = (e.bookings || []).reduce((sum, b) => sum + Number(b.totalPrice || 0), 0);
      const eventTickets = (e.bookings || []).reduce((sum, b) => sum + (b.quantity || 0), 0);
      const colors = ['bg-brand-orange', 'bg-blue-500', 'bg-purple-500', 'bg-emerald-500', 'bg-rose-500'];
